(() => {
  ...
  const API_BASE =
    'https://n8n-pi.taild8d05f.ts.net/webhook';

  const SAVE_URL =
    `${API_BASE}/workplan-time-plan-save`;

  const TEMPLATE_MANAGE_URL =
    `${API_BASE}/workplan-shift-template-manage`;
  ...
  function ensureConfirmModal() {
    ...
    backdrop.innerHTML = `
      <section
        class="wp-confirm-card"
        role="dialog"
        aria-modal="true"
        aria-labelledby="workplanConfirmTitle"
      >
        <div class="wp-confirm-icon">!</div>

        <h3 id="workplanConfirmTitle">
          Potwierdź operację
        </h3>
    ...
    backdrop
      .querySelector('.wp-confirm-cancel')
      .addEventListener(
        'click',
        () => finish(false)
      );
  ...
  function parseAssignedPlan(button) {
    const pill =
      button.closest('.assigned-pill');

    const card =
      button.closest('.demand-card');

    const shiftText =
      pill?.querySelector('em')?.textContent || '';
  ...
  async function removeAssignedPerson(button) {
    ...
    if (
      ...
      setPageMessage(
        'Nie udało się odczytać danych przypisania.',
        true
      );
    ...
    const accepted = await appConfirm({
      ...
      message:
        `${plan.name} zostanie usunięty ` +
        `z planu na ${date}. ` +
        `Poprzednia wersja pozostanie ` +
        `w historii zmian.`,
    ...
    try {
      ...
      /*
       * Ta operacja zapisuje wyłącznie zmianę
       * przypisania pracownika.
       *
       * Nie wysyła totalHours ani shiftHours.
       */
      ...
      setPageMessage(
        'Pracownik został usunięty z planu.'
      );
  ...
  async function removeTemplate(button) {
    ...
    const accepted = await appConfirm({
      ...
      message:
        ...
        `Istniejące plany i konfiguracje dni ` +
  ...
  /*
   * Patch obsługuje już wyłącznie własne
   * okna potwierdzające usuwanie.
   *
   * Nie obsługuje zapotrzebowania.
   * Nie pobiera ponownie planu.
   * Nie używa MutationObserver.
   * Nie zmienia inputów godzinowych.
   */
  document.addEventListener(
    'click',
    (event) => {
